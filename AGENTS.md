...
Terapkan prinsip berikut di **setiap** perubahan kode:

- **DRY**: satu logika = satu tempat. Tidak ada duplikasi. Ekstrak pola berulang ke fungsi/class reusable.
- **Nama ekspresif**: fungsi = kata kerja (`getUser`), variabel = kata benda (`$totalPrice`), boolean = prefix (`isActive`).
- **Satu tanggung jawab**: satu fungsi = satu pekerjaan. Jika perlu komentar untuk menjelaskan blok, ekstrak itu ke fungsi bernama.
- **Struktur folder rapi**: pisahkan kode sesuai concern masing-masing (Controllers, Models, Views, Services, dll). Satu folder = satu tanggung jawab. Jangan campur logic berbeda dalam satu direktori.
- **Konsisten**: ikuti pattern yang sudah ada di codebase (framework, struktur file, naming).
- **Komentar minimal**: kode harus self-documenting. Komentar hanya untuk *why*, bukan *what*.
- **Early return**: kurangi nesting dengan return awal.
- **Test**: pola Arrange → Act → Assert, satu asersi logis per test.
<!-- clean-code:end -->
