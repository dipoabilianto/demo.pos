...
@section('content')
<form method="POST" action="{{ route('products.update', $product) }}" class="max-w-2xl space-y-6" enctype="multipart/form-data">
    @csrf @method('PUT')
    <div class="rounded-2xl bg-white p-6 shadow-md shadow-warm-900/5 border border-warm-200/50 space-y-5">
        ...
        <div>
            ...
            <div class="flex items-center gap-4">
                @if ($product->image_url)
                    <img src="{{ $product->image_url }}" width="80" height="80" loading="lazy" class="h-20 w-20 object-cover rounded-xl border border-warm-200">
                @endif
                <label class="flex cursor-pointer items-center gap-2 rounded-xl border-2 border-dashed border-warm-300 px-4 py-3 text-sm text-warm-500 hover:border-theme-primary hover:text-theme-primary transition-colors flex-1" id="product-image-label">
                    ...
                    <span id="product-image-text">Ganti Foto</span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="block w-full rounded-xl border-warm-200 px-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
            </div>
            <div>
                <label class="block text-sm font-medium text-warm-700 mb-1.5">SKU</label>
                <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" class="block w-full rounded-xl border-warm-200 px-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
            </div>
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="block w-full rounded-xl border-warm-200 px-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">{{ old('description', $product->description) }}</textarea>
        </div>
    </div>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-md shadow-warm-900/5 border border-warm-200/50 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="price" value="{{ old('price', $product->price) }}" required step="0.01" min="0" class="block w-full rounded-xl border-warm-200 pl-10 pr-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
                </div>
            </div>
            <div>
                ...
                <div class="relative">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-warm-400 font-medium">Rp</span>
                    <input type="number" name="sale_price" value="{{ old('sale_price', $product->sale_price) }}" step="0.01" min="0" class="block w-full rounded-xl border-warm-200 pl-10 pr-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
                </div>
            ...
            <div>
                ...
                <div class="relative">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-warm-400 font-medium">Rp</span>
                    <input type="number" name="cost_price" value="{{ old('cost_price', $product->cost_price) }}" step="0.01" min="0" class="block w-full rounded-xl border-warm-200 pl-10 pr-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
                </div>
            </div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="number" name="stock" id="stock-input" value="{{ old('stock', $product->stock) }}" required min="0" class="block w-full rounded-xl border-warm-200 px-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
            </div>
            <div>
                <label class="block text-sm font-medium text-warm-700 mb-1.5" id="min-stock-label">Min. Stok</label>
                <input type="number" name="min_stock" id="min-stock-input" value="{{ old('min_stock', $product->min_stock) }}" min="0" class="block w-full rounded-xl border-warm-200 px-4 py-2.5 shadow-sm focus:border-theme-primary focus:ring-theme-primary/20 text-sm bg-white">
            </div>
        </div>
        <label class="flex items-center gap-2 cursor-pointer pt-2">
            <input type="checkbox" name="is_unlimited" value="1" {{ old('is_unlimited', $product->is_unlimited) ? 'checked' : '' }}
        ...
        <div class="flex items-center gap-3 pt-2">
            <label class="relative inline-flex items-center cursor-pointer">
                ...
                <span class="ml-3 text-sm text-warm-700 font-medium">Produk Aktif</span>
            </label>
        </div>
    </div>
