...
<div class="space-y-6 p-6">
    <div class="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-warm-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-base font-semibold text-warm-900">Daftar Pengguna</h3>
                <p class="text-xs text-warm-400">{{ $users->total() }} pengguna terdaftar</p>
            </div>
        </div>
        @if(auth()->user()->hasPermission('users.create'))
        <button @click="$dispatch('open-modal', 'add-user')"
            class="rounded-xl bg-theme-gradient-r px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-theme-shadow hover:opacity-90 transition-all">
            + Tambah Pengguna
        </button>
        @endif
    </div>
                ...
                <tbody class="divide-y divide-warm-100">
                    @forelse ($users as $user)
                        <tr class="hover:bg-warm-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-sm text-warm-600">{{ $user->branch?->name ?? '-' }}</span>
                            </td>
...
>
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4 pointer-events-none" x-transition:enter-end="opacity-100 translate-y-0 pointer-events-auto" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0 pointer-events-auto" x-transition:leave-end="opacity-0 translate-y-4 pointer-events-none" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{ route('settings.users.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="password" name="password" inputmode="text" required
                        class="block w-full rounded-xl border-warm-300 px-4 py-2.5 text-sm placeholder-warm-400 focus:border-theme-primary focus:ring-2 focus:ring-theme-primary/20 transition-all"
                        placeholder="Minimal 6 karakter">
                </div>

                <div>
                    <label class="block text-sm font-medium text-warm-700 mb-1.5">Cabang</label>
                    <x-ui.select name="branch_id" placeholder="-- Pilih Cabang --"
                        :options="$branches->pluck('name', 'id')"
                        class="rounded-xl border-warm-300 px-4 py-2.5 text-sm" />
                </div>
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4 pointer-events-none" x-transition:enter-end="opacity-100 translate-y-0 pointer-events-auto" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0 pointer-events-auto" x-transition:leave-end="opacity-0 translate-y-4 pointer-events-none" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{ route('settings.users.update', $user) }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-warm-700 mb-1.5">Nama</label>
                        <input type="text" name="name" value="{{ $user->name }}" inputmode="text" required
                            class="block w-full rounded-xl border-warm-300 px-4 py-2.5 text-sm placeholder-warm-400 focus:border-theme-primary focus:ring-2 focus:ring-theme-primary/20 transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-warm-700 mb-1.5">Email</label>
                        <input type="email" name="email" value="{{ $user->email }}" inputmode="email" required
                            class="block w-full rounded-xl border-warm-300 px-4 py-2.5 text-sm placeholder-warm-400 focus:border-theme-primary focus:ring-2 focus:ring-theme-primary/20 transition-all">
                    </div>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-warm-700 mb-1.5">Cabang</label>
                    <x-ui.select name="branch_id" placeholder="-- Pilih Cabang --"
                        :options="$branches->pluck('name', 'id')" :value="$user->branch_id"
                        class="rounded-xl border-warm-300 px-4 py-2.5 text-sm" />
                </div>
