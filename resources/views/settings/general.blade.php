...
@php
    ...
    if ($user->hasPermission('shifts.view')) $allowedTabs[] = 'shifts';
...
<div>
    <div class="mb-6 flex gap-1 overflow-x-auto rounded-xl bg-white p-1.5 shadow-sm border border-warm-200/50 max-w-full lg:w-fit">
        ...
        @endif
        @if($user->hasPermission('shifts.view'))
        <a href="{{ request()->fullUrlWithQuery(['tab' => 'shifts']) }}" class="shrink-0 rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200 {{ $tab === 'shifts' ? 'bg-theme-primary text-white shadow-sm' : 'text-warm-600 hover:bg-warm-100' }}">
            <div class="flex items-center gap-2">
    ...
    <form method="POST" action="{{ route('settings.update') }}">
        ...
        @if($user->hasPermission('settings.payment') && $tab === 'payment')
            @if($user->hasPermission('payment-methods.view'))
            <div class="rounded-2xl bg-white shadow-md shadow-warm-900/5 border border-warm-200/50 overflow-hidden mb-6">
                <div class="flex items-center gap-3 px-6 py-4 border-b border-warm-100">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-theme-primary/10 text-theme-primary">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z" /></svg>
                    </div>
                    <div>
                        <h3 class="text-base font-semibold text-warm-900">Metode Pembayaran</h3>
                        <p class="text-xs text-warm-400">Aktifkan atau nonaktifkan metode pembayaran yang tersedia untuk pelanggan.</p>
                    </div>
                </div>
                <div class="grid gap-4 p-6 md:grid-cols-2">
                    @php $paymentMethodGroups = $paymentMethods->groupBy('group'); @endphp
                    @foreach (['offline' => 'Offline (Bayar Langsung)', 'virtual_account' => 'Virtual Account', 'ewallet' => 'E-Wallet', 'bank_transfer' => 'Bank Transfer', 'retail' => 'Ritel', 'over_the_counter' => 'OTC', 'qris' => 'QRIS'] as $groupKey => $groupLabel)
                        @if ($paymentMethodGroups->has($groupKey))
                            <div class="rounded-xl border border-warm-200/70 overflow-hidden">
                                <div class="px-4 py-2.5 border-b border-warm-100 bg-warm-50/50">
                                    <h4 class="text-xs font-semibold text-warm-700">{{ $groupLabel }}</h4>
                                </div>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-md shadow-warm-900/5 border border-warm-200/50 space-y-5">
                ...
                <div class="rounded-xl bg-theme-primary/10 border border-theme-primary/20 p-4 text-sm text-theme-primary flex items-start gap-3">
                    ...
                    <span>Prioritas: <code>.env</code> > Database (form ini).</span>
                </div>
    ...
    @if ($user?->hasPermission('shifts.view') && $tab === 'shifts')
        @include('settings.tabs.shifts', ['branches' => $shiftBranches, 'dayNames' => $dayNames])
    @endif
</div>
@endsection
