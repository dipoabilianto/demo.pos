...
<div>
    ...
    <form method="POST" action="{{ route('settings.update') }}">
        ...
        <div x-data="{ items: {!! htmlspecialchars(json_encode(old('promotions', $promotionsForTarget ?? [])), ENT_QUOTES, 'UTF-8') !!}, uploading: false, uploadPromoImage(event, item, i) { const file = event.target.files[0]; if (!file) return; this.uploading = true; const form = new FormData(); form.append('image', file); form.append('promo_id', item.id); form.append('_token', document.querySelector('meta[name=\'csrf-token\']').content); fetch('{{ route('settings.upload-promo-image') }}', { method: 'POST', body: form }).then(r => r.json()).then(d => { if (d.success) { item.image = d.path; } this.uploading = false; }).catch(() => { this.uploading = false; }); } }">
            <div class="rounded-2xl bg-white p-6 shadow-md shadow-warm-900/5 border border-warm-200/50 space-y-5">
