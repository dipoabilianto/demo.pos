...
<div class="mb-6 flex gap-1 overflow-x-auto rounded-xl bg-white p-1.5 shadow-sm border border-warm-200/50 max-w-full lg:w-fit">
    <a href="{{ request()->fullUrlWithQuery(['tab' => 'list']) }}"
       class="shrink-0 rounded-lg px-5 py-2 text-sm font-medium transition-all duration-200 {{ $tab === 'list' ? 'bg-theme-primary text-white shadow-sm' : 'text-warm-600 hover:bg-warm-100' }}">
    ...
    <a href="{{ request()->fullUrlWithQuery(['tab' => 'opname']) }}"
       class="shrink-0 rounded-lg px-5 py-2 text-sm font-medium transition-all duration-200 {{ $tab === 'opname' ? 'bg-theme-primary text-white shadow-sm' : 'text-warm-600 hover:bg-warm-100' }}">
        <div class="flex items-center gap-2">
